            ...
            <c:if test="${status ne null }">
                <c:choose>
                    <c:when test="${status eq 'sendrecoverpassword'}">
                        <p style="color: green"> – Ihr Passwort wurde wiederhergestellt. Das Passwort wird Ihnen per Email an die genannte Anschrift gesendet</p>
                    </c:when>
                            ...
                            <c:forEach items="${stars}" var="star" varStatus="loop">
                                <c:choose>
                                    <c:when test="${loop.last}">
                                        <li style="text-align: left; display: table; height: auto; margin-left: 15px; margin-top: 15px"><div style="float: left; margin-right: 15px" id="box5"><a href="/orderingstar?id=${star.id}"><img  style="border: 1px solid #d4d1d5; border-radius: 3px; padding: 2px;" width="60" height="auto" src="/starSource/${star.id}/mainphoto.jpg" ></a></div><a href="/orderingstar?id=${star.id}" style="display: table-cell; vertical-align: middle" class ="ids" idstar="${star.id}" names="${star.name} <c:if test="${star.alias != null}">(${star.alias})</c:if>">${star.name} <c:if test="${star.alias != null}">(${star.alias})</c:if></a>
                                    </c:when>
                                    <c:otherwise>
